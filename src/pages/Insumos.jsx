...
export default function Insumos() {
  ...
  const [formData, setFormData] = useState({
    ...
    factorConversion: '', ubicacion: '', stock: '',
  });
...
const handleExcelUpload = async (e) => {
  ...
  reader.onload = async (event) => {
    ...
    // 1. Validar que todos tengan código
    const filasSinCodigo = rows
      .map((row, index) => ({ index: index + 2, codigo: row.codigo })) // fila +2 (por encabezado)
...
if (filasSinCodigo.length > 0) {
  ...
  return; // 🚫 Detener proceso si hay error
}

    // 2. Obtener insumos actuales
    const snapshot = await getDocs(collection(db, 'insumos'));
          ...
          <tbody>
            {insumos.map(insumo => (
              <tr key={insumo.id} className="border-t hover:bg-[#F5F4F1]">
                ...
                <td className="px-4 py-2">{insumo.stock}</td>
